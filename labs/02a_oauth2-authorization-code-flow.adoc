...
* `response_type`: Defines that the response should contains an _Authorization Code_ (and not an _Access Token_).
* `client_id`: The public identification of the Client. Must be registered in previously Keyloak and must be unique per Realm.
...
. The user is redirected back to the application and the issued Authorization Code is provided as a parameter in the callback URL.
+
[source,http]
----
HTTP/1.1 302 Found
